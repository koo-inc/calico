...
public class FmStorage implements MediaStorage {
  @Inject
  private FmStorageSetting setting;
  ...
  public Optional<Media> get(String id) {
    ...
    return meta.map(m -> {
      ...
      param.put("path", setting.getBasePath() + id);
  ...
  public MediaProxy store(Media media, Function<Media, String> idGenerator) {
    ...
    Map<String, Object> param = Maps.newHashMap();
    param.put("path", setting.getBasePath() + proxy.getId());
  ...
  public void remove(Media media) {
    try {
      Map<String, Object> param = Maps.newHashMap();
      param.put("path", setting.getBasePath() + media.getId());
  ...
  */
  ...
  public List<String> getIdList(String idPartial) {
    Map<String, Object> param = Maps.newHashMap();
    String basePath = setting.getBasePath();
