...
abstract class FmStorageCommand<T> {
  ...
  protected String encodeUrl(String str) {
    return URLEncoder.encode(str, FmStorageEnv.getEnv().getCharset());
  }
  @SneakyThrows
  protected String decodeUrl(String str) {
    return URLDecoder.decode(str, FmStorageEnv.getEnv().getCharset());
  }

  @SneakyThrows
  protected URL createGetUrl(Map<String, Object> param) {
    return new URL(FmStorageEnv.getEnv().getUrl() + action + "?" + toParamString(param));
  }
  @SneakyThrows
  protected URL createPostUrl() {
    return new URL(FmStorageEnv.getEnv().getUrl() + action);
  }
